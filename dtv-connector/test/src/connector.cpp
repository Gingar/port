...
public:
	...
	connector::Connector *startServer( const std::string &pipe, int msConnectTimeout=5000 ) {
		...
		srv->onConnect( boost::bind( &ConnectorTest::onConnect, this, test::flags::connector_ev_srv ) );
		srv->onConnectTimeout( boost::bind( &ConnectorTest::onConnectTimeout, this, test::flags::connector_ev_srv ) );
		srv->onDisconnect( boost::bind( &ConnectorTest::onDisconnect, this, test::flags::connector_ev_srv ) );
		srv->onTimeout( boost::bind( &ConnectorTest::onTimeout, this, test::flags::connector_ev_srv ) );
	...
	connector::Connector *startClient( const std::string &pipe, int maxRetry=10, int timeout=50 ) {
		...
		cli->onConnect( boost::bind( &ConnectorTest::onConnect, this, test::flags::connector_ev_rcv ) );
		cli->onConnectTimeout( boost::bind( &ConnectorTest::onConnectTimeout, this, test::flags::connector_ev_rcv ) );
		cli->onDisconnect( boost::bind( &ConnectorTest::onDisconnect, this, test::flags::connector_ev_rcv ) );
		return cli;
	}
...
TEST_F( ConnectorTest, basic_connect ) {
	start();
	stop();
...
TEST_F( ConnectorTest, client_connect_connectTimeout ) {
	int msTimeout = 50;
	int retry = 2;
...
TEST_F( ConnectorTest, server_connect_connectTimeout ) {
	int msTimeout = 100;
...
TEST_F( ConnectorTest, send_without_connect ) {
...
TEST_F( ConnectorTest, check_keep_alive ) {
...
TEST_F( ConnectorTest, check_keep_alive_timeout ) {
	int keepInterval=500;
	start();
...
TEST_F( ConnectorTest, check_handler ) {
	start();
...
public:
	...
	virtual void process (util::Buffer* message) {
		util::WORD  msgSize = MSG_PAYLOAD_SIZE( (unsigned char*) message->buffer());
		util::BYTE *payload = MSG_PAYLOAD(message->buffer());
	...
	virtual void send (connector::Connector* conn, util::Buffer* msg) {
		util::Buffer buf;
		buf.copy(3, msg->buffer(), msg->length());
		MAKE_MSG(buf, connector::messages::keyRegister, msg->length()+3);
		conn->send(&buf);
	}
...
TEST_F( ConnectorTest, dummyMessageEmptyMessage ) {
	start();
...
TEST_F( ConnectorTest, dummyOneCharMessage) {
	start();
	DummyHandler *dummy = new DummyHandler();
	...
	EXPECT_EQ(dummy->received->buffer()[0], '\xA5');
	...
	EXPECT_EQ(dummy->received->length(), 1);
	EXPECT_EQ(dummy->received->buffer()[0], '\x5A');
}

TEST_F( ConnectorTest, dummyOneByteLengthLimit) {
	start();
	DummyHandler *dummy = new DummyHandler();
	char text[258];
...
/*TEST_F( ConnectorTest, dummyTwoBytesLengthLimit) {
	start();
	DummyHandler *dummy = new DummyHandler();
	char *text;
...
static void keyRegisterBasic( std::set< util::key::type > *keys, connector::KeyRegisterData *m ) {
	*keys = m->keys;
	test::notify( test::flags::connector_ev_data );
}

TEST_F( ConnectorTest, key_register_basic ) {
	start();
...
TEST_F( ConnectorTest, key_register_all ) {
	start();
	...
	connector::KeyRegisterHandler key;
	std::set< util::key::type > keys;
#define SAVE_KEY( c, n ) keys.insert( static_cast<util::key::type>(util::key::c) );
			KEY_LIST(SAVE_KEY);
#undef SAVE_KEY
	key.send( _client, keys );
	test::wait( test::flags::connector_ev_data );
...
TEST_F( ConnectorTest, key_event_basic ) {
	start();
...
TEST_F( ConnectorTest, start_event_basic ) {
	start();
...
void videoResize_handler( struct VideoEvent *ev, connector::AVDescriptor *descriptor ) {
...
TEST_F( ConnectorTest, video_basic ) {
	start();

	struct VideoEvent ev;
	connector::VideoResizeHandler* videoResizeHandler = new connector::VideoResizeHandler();
	videoResizeHandler->onVideoResizeEvent( boost::bind( &videoResize_handler, &ev, _1 ) );
	_server->addHandler( videoResizeHandler);

	connector::VideoResizeHandler videoResize;
	connector::AVDescriptor *descriptor = new connector::AVDescriptor(0.25,0.25,0.50,0.50);
	videoResize.send( _client, descriptor);
	test::wait( test::flags::connector_ev_data );
...
/*
TEST_F( ConnectorTest, exit_basic ) {
	start();
...
void editingCommandHandler_callback (connector::EditingCommandData *where, connector::EditingCommandData *received) {
	where->copyFrom( received );
	test::notify( test::flags::connector_ev_data);
}

TEST_F( ConnectorTest, editingcommand_basic ) {
...
TEST_F( ConnectorTest, editingcommand_fullOneSection ) {
...
TEST_F( ConnectorTest, editingcommand_fullManySections ) {
...
TEST_F( ConnectorTest, editingcommand_one_full_section ) {
	start();
	connector::EditingCommandHandler* ecHandler = new connector::EditingCommandHandler();
	connector::EditingCommandData ec_received; // Where I keep received data...
...
TEST_F( ConnectorTest, editingcommand_npt ) {
...
TEST_F( ConnectorTest, editingcommand_eventID ) {
	const unsigned char privateData[] = "";
	start();
	connector::EditingCommandHandler* ecHandler = new connector::EditingCommandHandler();
	connector::EditingCommandData ec_received; // Where I keep received data...
...
TEST_F( ConnectorTest, editingcommand_commandTag ) {
	const unsigned char privateData[] = "";
	start();
	connector::EditingCommandHandler* ecHandler = new connector::EditingCommandHandler();
	connector::EditingCommandData ec_received; // Where I keep received data...
...
TEST_F( ConnectorTest, editingcommand_invalidSeqNum01 ) {
	start();
	connector::EditingCommandHandler* ecHandler = new connector::EditingCommandHandler();
	connector::EditingCommandData ec_received; // Where I keep received data...
...
TEST_F( ConnectorTest, editingcommand_invalidSeqNum02 ) {
	start();
	connector::EditingCommandHandler* ecHandler = new connector::EditingCommandHandler();
	connector::EditingCommandData ec_received; // Where I keep received data...
...
TEST_F( ConnectorTest, editingcommand_invalidSeqNum03 ) {
	start();
	connector::EditingCommandHandler* ecHandler = new connector::EditingCommandHandler();
	connector::EditingCommandData ec_received; // Where I keep received data...
...
TEST_F( ConnectorTest, editingcommand_invalidEditingCommand ) {
	start();
	connector::EditingCommandHandler* ecHandler = new connector::EditingCommandHandler();
	connector::EditingCommandData ec_received; // Where I keep received data...
...
TEST_F( ConnectorTest, canvasHandler_basic ) {
	int received = 0;
	start();
