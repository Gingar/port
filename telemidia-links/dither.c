...
void dither_4(unsigned short *, struct bitmap *, int *);     /* DITHER_TEMPLATE */
void round_4(unsigned short *, struct bitmap *);             /* ROUND_TEMPLATE */
long color_332(int);
long color_121(int);
...
#define SAVE_CODE *outp=bt>>16;\
	...
	outp[3]=0xff;\
	outp+=4;
DITHER_TEMPLATE(dither_4)
ROUND_TEMPLATE(round_4)
#undef SAVE_CODE
#undef SKIP_CODE 
...
void init_dither(int depth)
{
	switch(depth){
		 ...
		 * B G R A
		 */
		printf("case 4: B G R A\n");
		make_red_table(8,0,0,0);
		make_green_table(8,0,0,0);
		make_blue_table(8,0,0,0);
		dither_fn_internal=dither_4;
		round_fn=round_4;
		break;
