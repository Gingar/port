...
namespace io {
	...
	{
		connector = c;
		keyHandler = new connector::KeyRegisterHandler();

		//	Add key event handler
		connector::KeyEventHandler *keyEventHandler = new connector::KeyEventHandler();
		keyEventHandler->onKeyEvent( boost::bind( &InputManager::onZapperKeyRcv, this, _1, _2 ) );
		c->addHandler( keyEventHandler );
	}
	...
	void InputManager::addInputEventListener(
		...
		if (notifying) {
			...
			if (i != eventListeners->end()) {
				iEvs = i->second;
				if (util::compareSets(iEvs,evs)) { // are the keys we are listening are the same we were listening??
				    unlock();           // Do nothing
				    evs->clear();
				    delete evs;
				    return ;
	...
	void InputManager::postEvent(int keyCode, bool isUp) {
		IInputEvent* ie;

#if HAVE_COMPSUPPORT
		ie = ((InputEventCreator*)(cm->getObject("InputEvent")))(NULL, keyCode);
#else
		ie = new DFBGInputEvent(keyCode, isUp);
#endif
		postEvent(ie);
	}

	void InputManager::onZapperKeyRcv( util::key::type key, bool isUp ) {
		postEvent( CodeMap::keyToValue(key), isUp );
	}
	...
	void InputManager::run() {
		...
		while (running && eventBuffer != NULL) {
			...
			while (inputEvent != NULL) {
				...
				if (inputEvent->isPressedType() && ((util::getCurrentTimeMillis() -
