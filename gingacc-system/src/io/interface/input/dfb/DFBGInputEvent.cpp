...
namespace io {
	...
	DFBGInputEvent::DFBGInputEvent(const int keyCode, bool isUp) {
...
extern "C" IInputEvent* createDFBInputEvent(void* event, const int symbol, bool isUp) {
	if (event != NULL) {
		return new DFBGInputEvent(event);
	}

	if (symbol >= 0) {
		return new DFBGInputEvent(symbol, isUp);
	}
